...
export function registerProjectHandlers() {
		...
		try {
			...
			try {
				...
				return {
					success: true,
					message: tElectron(
						"project.revealFallback",
						"Could not reveal item, but opened directory.",
					),
				};
			} catch (openError) {
		...
		try {
			...
			if (openPathResult) {
				return {
					...
					message: tElectron(
						"project.openRecordingsFolderFailed",
	...
	ipcMain.handle("choose-recordings-directory", async () => {
		try {
			...
			const result = await dialog.showOpenDialog({
				title: tElectron("project.chooseRecordingsFolder", "Choose recordings folder"),
				defaultPath: current,
			...
			return {
				...
				error: String(error),
				message: tElectron(
					"project.setRecordingsFolderFailed",
					"Failed to set recordings folder",
				),
			};
		}
	});
			...
			try {
				...
				if (trustedExistingProjectPath) {
					...
					return {
						...
						message: tElectron("project.saved", "Project saved successfully"),
					};
				}
				...
				if (result.canceled || !result.filePath) {
					return {
						success: false,
						canceled: true,
						message: tElectron("project.saveCanceled", "Save project canceled"),
					};
				}
				...
				return {
					success: false,
					message: tElectron("project.saveFailed", "Failed to save project file"),
					error: String(error),
				};
			}
		},
		...
		async (
			...
			try {
				...
				if (!normalizedProjectName) {
					return {
						success: false,
						message: tElectron("project.nameRequired", "Project name is required"),
					};
				}
		...
		try {
			...
			if (result.canceled || result.filePaths.length === 0) {
				return {
					success: false,
					canceled: true,
					message: tElectron("project.openCanceled", "Open project canceled"),
				};
			}
			...
			return {
				success: false,
				message: tElectron("project.loadFailed", "Failed to load project file"),
				error: String(error),
			};
		}
	});
	...
	ipcMain.handle("load-current-project-file", async () => {
		try {
			if (!currentProjectPath) {
				return {
					success: false,
					message: tElectron("project.noActiveProject", "No active project"),
				};
			}
			...
			return {
				success: false,
				message: tElectron(
					"project.loadCurrentFailed",
					"Failed to load current project file",
				),
				error: String(error),
			};
		}
	...
	ipcMain.handle("open-project-file-at-path", async (_, filePath: string) => {
		try {
			...
			return {
				success: false,
				message: tElectron("project.openFileFailed", "Failed to open project file"),
				error: String(error),
			};
		}
	});
		...
		try {
			...
			if (openPathResult) {
				return {
					...
					error: openPathResult,
					message: tElectron(
						"project.openProjectsFolderFailed",
						"Failed to open projects folder.",
					),
				};
			}
