...
export function registerRecordingHandlers(
		...
		async (_, source: SelectedSource, options?: NativeMacRecordingOptions) => {
			...
			if (process.platform === "win32") {
				...
				if (!windowsCaptureAvailable) {
					return {
						success: false,
						message: tElectron(
							"recording.windowsUnavailable",
							"Native Windows capture is not available on this system.",
						),
					};
				}
				...
				if (windowsCaptureProcess) {
					return {
						success: false,
						message: tElectron(
							"recording.windowsAlreadyActive",
							"A native Windows screen recording is already active.",
						),
					};
				}
				...
				try {
					...
					return {
						success: false,
						message: tElectron(
							"recording.windowsStartFailed",
							"Failed to start native Windows capture",
						),
						error: String(error),
					};
				}
			...
			if (process.platform !== "darwin") {
				return {
					success: false,
					message: tElectron(
						"recording.nativeOnlyMac",
						"Native screen recording is only available on macOS.",
					),
				};
			}
			...
			if (nativeCaptureProcess) {
				return {
					success: false,
					message: tElectron(
						"recording.nativeAlreadyActive",
						"A native screen recording is already active.",
					),
				};
			}
			...
			try {
				...
				if (
					...
					const { response } = await dialog.showMessageBox({
						type: "warning",
						title: tElectron(
							"recording.screenPermissionTitle",
						...
						buttons: [
							...
							tElectron("recording.cancel", "Cancel"),
						],
						defaultId: 0,
						cancelId: 1,
					});
				...
				if (errorStr.includes("MICROPHONE_PERMISSION_DENIED")) {
					const { response } = await dialog.showMessageBox({
						type: "warning",
						title: tElectron(
							"recording.microphonePermissionTitle",
		...
		try {
			...
			if (process.platform === "win32" && windowsNativeCaptureActive) {
				...
				try {
					...
					return {
						success: false,
						message: tElectron(
							"recording.windowsStopFailed",
							"Failed to stop native Windows capture",
						),
						error: String(error),
					};
				}
			...
			if (!nativeScreenRecordingActive) {
				...
				return {
					success: false,
					message: tElectron(
						"recording.nativeNotActive",
						"No native screen recording is active.",
					),
				};
			}
		...
		if (process.platform === "win32") {
			if (!windowsNativeCaptureActive || !windowsCaptureProcess) {
				return {
					success: false,
					message: tElectron(
						"recording.windowsNotActive",
						"No native Windows screen recording is active.",
					),
				};
			}
			...
			try {
				...
				return {
					success: false,
					message: tElectron(
						"recording.windowsPauseFailed",
						"Failed to pause native Windows capture",
					),
					error: String(error),
				};
			}
		...
		if (process.platform === "win32") {
			...
			try {
				...
				return {
					success: false,
					message: tElectron(
						"recording.windowsResumeFailed",
						"Failed to resume native Windows capture",
					),
					error: String(error),
				};
			}
		...
		try {
			...
			if (!videoPath) {
				return {
					success: false,
					message: tElectron(
						"recording.windowsMuxPendingMissing",
						"No native Windows video pending for mux",
					),
				};
			}
				...
				try {
					...
					try {
						...
						return {
							...
							path: videoPath,
							message: tElectron(
								"recording.windowsMuxFailed",
								"Failed to mux native Windows recording",
							),
							error: String(error),
						};
					} catch {
	...
	ipcMain.handle("start-ffmpeg-recording", async (_, source: SelectedSource) => {
		if (ffmpegCaptureProcess) {
			return {
				success: false,
				message: tElectron(
					"recording.ffmpegAlreadyActive",
					"An FFmpeg recording is already active.",
				),
			};
		}
	...
	ipcMain.handle("stop-ffmpeg-recording", async () => {
		if (!ffmpegScreenRecordingActive) {
			return {
				success: false,
				message: tElectron("recording.ffmpegNotActive", "No FFmpeg recording is active."),
			};
		}
		...
		try {
			...
			return {
				success: false,
				message: tElectron("recording.ffmpegStopFailed", "Failed to stop FFmpeg recording"),
				error: String(error),
			};
		}
	});
	...
	ipcMain.handle("store-recorded-video", async (_, videoData: ArrayBuffer, fileName: string) => {
		try {
			...
			return {
				success: false,
				message: tElectron("recording.storeVideoFailed", "Failed to store video"),
				error: String(error),
			};
		}
	});
	...
	ipcMain.handle("get-recorded-video-path", async () => {
		try {
			...
			if (sortedCandidates.length === 0) {
				return {
					success: false,
					message: tElectron("recording.noRecordedVideo", "No recorded video found"),
				};
			}
			...
			return {
				success: false,
				message: tElectron(
					"recording.noUsableRecordedVideo",
					"No usable recorded video found",
				),
			};
		} catch (error) {
			...
			return {
				success: false,
				message: tElectron("recording.getVideoPathFailed", "Failed to get video path"),
				error: String(error),
			};
		}
	});
	...
	ipcMain.handle("set-recording-state", (_, recording: boolean) => {
		...
		const source = selectedSource || { name: tElectron("system.screen", "Screen") };
	...
	ipcMain.handle("get-cursor-telemetry", async (_, videoPath?: string) => {
		...
		try {
			...
			return {
				success: false,
				message: tElectron(
					"recording.cursorTelemetryLoadFailed",
					"Failed to load cursor telemetry",
				),
				error: String(error),
				samples: [],
			};
		...
		async (_, videoPath: string | undefined, samples: CursorTelemetryPoint[]) => {
			...
			if (!targetVideoPath) {
				return {
					...
					samples: [],
					message: tElectron(
						"recording.cursorTelemetryNoVideoPath",
						"No video path available for cursor telemetry",
					),
			...
			try {
				...
				return {
					...
					samples: [],
					message: tElectron(
						"recording.cursorTelemetrySaveFailed",
						"Failed to save cursor telemetry",
					),
					error: String(error),
				};
			}
